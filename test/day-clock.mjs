...
//   4. The ratified 2026-07-30 arithmetic: stage ladder at 14/16/18 h awake (energy 4/2/0),
//      coffee +5 now / −3 at wake (net positive), r(vibe) with average = full-from-empty in 8 h
//      and best = exactly 2× worst, the tired signal blink firing exactly once per crossing.
//
...
check('pinned: energy ladder 18/4/2', DAY_PARAMS.fullEnergyH === 18 && DAY_PARAMS.sleepyAtH === 4 && DAY_PARAMS.tiredAtH === 2)
check('pinned: coffee +5/−3', DAY_PARAMS.coffeeReliefH === 5 && DAY_PARAMS.coffeeDebtH === 3)
check('pinned: sleep rates 1.5/3.0', DAY_PARAMS.sleepRateWorstH === 1.5 && DAY_PARAMS.sleepRateBestH === 3.0)
...
// ── 2. Midnight is a day boundary; energy drains 1:1 ────────────────────────────────────────────
{
  ...
  check('18 h awake drains a full 18 h tank to 0', sys.energyH() === 0)
  check('energy floors at 0 (no deeper stage)', (sys.update(2), sys.energyH() === 0))
  DAY_PARAMS.dayLengthSec = 1440
}

// ── 3. The stage ladder at 14/16/18 h awake ─────────────────────────────────────────────────────
{
  const { sys } = mkDay()
  sys.start()
  ...
  for (let t = 0; t < 20; t += 0.01) {          // 0.01 s = 0.01 in-game h steps
    sys.update(0.01)
    const s = sys.stage()
    if (!(s in firstAt)) firstAt[s] = 18 - sys.energyH()   // hours awake at first sighting
  }
  const near = (a, b) => Math.abs(a - b) < 0.02
  check('sleepy at 14 h awake', near(firstAt.sleepy, 14), `at ${firstAt.sleepy}`)
  check('tired at 16 h awake', near(firstAt.tired, 16), `at ${firstAt.tired}`)
  check('exhausted at 18 h awake', near(firstAt.exhausted, 18), `at ${firstAt.exhausted}`)
  DAY_PARAMS.dayLengthSec = 1440
}
...
// ── 6. Coffee arithmetic ────────────────────────────────────────────────────────────────────────
{
  ...
  sys.start()
  DAY_PARAMS.dayLengthSec = 24
  sys.update(15)                                 // 15 h awake → 3 h left
...
// ── 7. Sleep: the ratified recovery arithmetic + the loan settled at wake ───────────────────────
{
  const { sys } = mkDay()
  sys.start()
  check('r(0.5) = 2.25 h/h (average site: 8 h = full from empty)', sys.recoveryRate(0.5) === 2.25)
  check('r(1) = 2 × r(0) (best site is exactly twice the worst)', sys.recoveryRate(1) === 2 * sys.recoveryRate(0))

  DAY_PARAMS.dayLengthSec = 24
  sys.update(18)                                 // empty, day 2, hour 01:00
  const dayBefore = runState.day
  sys.sleep(8, 0.5)                              // the ratified headline: average site, 8 h
  check('empty + 8 h at an average site = a full tank', sys.energyH() === DAY_PARAMS.fullEnergyH,
    `e=${sys.energyH()}`)
  ...
  sys.update(14)                                 // 09:00 + 14 h = 23:00, 4 h left
  sys.drinkCoffee()                              // 9 h left, debt 3
  ...
  // settled = 9 + 2.25·9 − 3 = 26.25 → clamps to 18: sleeping in DID pay the loan off.
  ...
  sys.update(16)                                 // 2 h left
  sys.drinkCoffee()                              // 7 h left, debt 3
  sys.sleep(2, 0)                                // settled = 7 + 1.5·2 − 3 = 7
  check('short sleep charges the debt once, uncapped', Math.abs(sys.energyH() - 7) < 1e-9, `e=${sys.energyH()}`)
