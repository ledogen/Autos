...
export const GATES = [
  ...
  { file: 'paper-tour.mjs', subsystem: 'story', cost: 'heavy', extraDeps: ['src/poi.js', 'src/mission.js'],
    desc: 'FEAT-61 the paper ROUND on a real network: a tour plans at every rung of the 4→9→12→15 ladder, ONE par over the whole thing (SM-INV-2) growing monotonically but not proportionally with the round, a lower tier is a PREFIX of a higher one (SM-INV-12 — the tier picks who is on the round, never what exists), a stop is an (edge, arc) POINT spliced into the graph so the round reaches the porch and carries on whichever way is shorter — never snapped to a junction (5 of 6 customers unapproached) and never a whole street driven end to end (ran past the porch to finish the tarmac, then turned around), 2-opt shortens the finished order, every segment names a registered edge, the driven polyline stays inside the region wall, deadlineFor(par) > par at every rung, degenerate inputs return null rather than a broken round — PLUS a whole round driven end to end through PaperRouteSystem: park at Larry\'s → offer → staging (the clock does NOT start on the pad) → leaving the threshold starts it → A PAPER DEAD CENTRE ON A ROUND CUSTOMER CREDITS THEM at q=1.00 and moves the counter (the mechanic owner-reported broken 2026-08-09), a second paper on the same porch is spent not double-counted, a paper on a customer who is not on this round scores nothing, the last porch ends the round, and a perfect round letters S, pays through settleFlat and earns the next rung' },
