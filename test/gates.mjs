...
export const GATES = [
  ...
  { file: 'paper-route.mjs', subsystem: 'story', cost: 'fast', extraDeps: [],
    desc: 'FEAT-61 paper route scoring: the accuracy law q(0)=1 → q(R)=0.30 linear with the rim cliff OUTSIDE, PARTIAL ROUTES PAY (1-of-9 letters D and still banks one paper — this mission is the income floor), flat rate anchored k·par·tier·paperW/customers so a perfect route at par pays 0.60× ONE DAY-TIER UNIT (re-anchored 2026-08-16 — it used to mean 0.60× the margin line at par, which halved when par stopped paying a full unit), expediency bonus unreachable without full coverage and capped at bonusMax, THE CLOCK grades on the par ratio ÷ coverage (accuracy pays the rate, never the letter) with par itself a C, ladder 4→9→12→15 advancing only on a perfect route, spares 100%→30%, the bell IS par (tolerance 1.0) so finishing at par settles no time money, SM-INV-4 untouched (payoutFor/gradeRun neither called nor imported) with settleFlat sharing one wallet + pointsFor, degenerate inputs pay 0 not NaN' },
