...
severity: minor
source: user-request
note: "Water feature, scoped not built. Lakes spawn in valleys at the local low point and fill the
bottom of the valley volume up to a water plane. Some valleys are deeper than others — that's fine,
each lake just locally fills its own basin. Roads ROUTE AROUND lakes (treated as an obstacle/no-go
...
# FEAT-17: Lakes — fill valley basins with water; roads route around them

## Context

The user wants standing-water features. A lake forms where a **valley has a local low point**: water
collects at the bottom and fills the basin up to a flat water plane. Valleys vary in depth and
elevation — each lake just **locally fills the bottom of its own valley**, so different lakes sit at
different absolute heights. Roads must **route around** a lake (it is an obstacle), and there is a
...
Sister ticket: **FEAT-18 rivers** (flowing water + bridges). Lakes are the *still-water / basin-fill*
side; rivers are the *channel-carve + bridge* side.

## Desired behaviour

- **Placement:** find local basins (valley low points) deterministically from the heightfield and seed.
  Not every valley needs a lake — some fraction qualify (controllable). The water surface is a flat
  plane at a chosen fill level for that basin.
- **Local fill:** the lake fills the basin volume up to its water plane (a horizontal surface). It does
  NOT need global hydrology — just locally flood the bottom of the valley. Different lakes → different
...
- **Water plane / shoreline:** the shoreline is the contour where raw terrain meets the water level.
  ...
  to that contour.
- **Road avoidance:** the road network treats the lake footprint + a **skirt** buffer as a no-go region
  and routes around it. Roads must not cross or touch a lake (rivers get bridges — lakes do not).
...
- **Deterministic + window-invariant:** lake set, basin levels, and shorelines are a pure function of
  seed/coords/params — same discipline as the road surface (no stream-order or draw-distance dependence;
  a lake looks identical regardless of where you approach it from).
...
- **Fill level per basin:** pick the water plane height (e.g. fixed depth below the rim / spill point, or
  a fraction of basin depth). Avoid lakes that overflow their basin. Min basin size to avoid puddles.
- **Terrain interaction:** does a lake CARVE the terrain (deepen/smooth the basin floor) or just render a
...
- **Road no-go integration:** where the router learns the obstacle. The graph builder (`src/road-graph.js`)
  ...
  Worker) need to reject/penalize edges entering the lake+skirt footprint. Decide: drop anchors inside
...
- **Skirt width:** a tunable shoreline buffer (`lakeSkirtWidth`) that excludes road gen and seeds scatter.
...
- **Physics:** out of scope for v1? (Driving into a lake = ??? — at minimum don't fall through to void.)
  Note the decision; full buoyancy/drag is not required for the first cut.

## Acceptance

- Some valleys contain a flat lake filling the basin bottom; different lakes sit at different heights,
  each correctly contained by its valley rim (no overflow, no floating water).
- Roads route cleanly AROUND every lake, with no road geometry inside the lake or its skirt buffer.
- A vegetated skirt rings each lake (trees/plants/rocks via FEAT-06 scatter), no road at the water's edge.
- Deterministic + window-invariant: a given lake renders identically regardless of approach direction or
  draw distance; `npm test` stays green (carve gates, smoothness, road-band coverage, route-worker sync).
- Tunable: lake frequency, min basin size, fill level, `lakeSkirtWidth` — debug sliders, USER-OWNED set.

## Related

- **FEAT-18** rivers (`feat-water-rivers.md`) — the flowing-water sister; rivers carve a channel and roads
  BRIDGE them (lakes are routed AROUND, not bridged). Share the deterministic water-placement + carve work.
...
- **FEAT-13** road network graph (`feat-road-network-graph.md`, [[project_feat13_v2_foundation]]) — the
  router/graph that must learn the lake no-go region.
