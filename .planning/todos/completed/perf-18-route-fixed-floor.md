...
created: 2026-07-14
closed: 2026-07-14
resolution: "Investigated to conclusion — no net-win item shipped (all 4 work items refuted/reverted; see STATUS section). The one surviving genuine lever (byte-identical incremental ancestor-proximity index over the in-search self-clearance walk, ~18.9 ms/edge) is folded into PERF-19 item 4, so PERF-18 has no independent value left. The unscoped repair-re-search lever (~20.3 ms/edge on ~14% dirty edges) is deferred — capture as a fresh narrow ticket if ever pursued (the corridor-kept-repair variant is moot post PERF-17 revert)."
source: user-request
