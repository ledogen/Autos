...
- [~] **Phase 9: Road Surface** — Ribbon mesh, worn asphalt, crown + camber, cut-and-fill terrain carve, 5-zone materials, merged at-grade intersections, physics height and normal. Plans 09-01..09 executed; verification found the carve-and-meet height-agreement gate violated + a perf regression. RE-ARCHITECTED via gap-closure 09-10..12: decal ribbon authoritative on top (polygonOffset + edge skirts), terrain carved cheaply BELOW the ribbon (perf restored), physics still rides the ribbon, new 3-clause exit gate (terrain-below + ribbon-driven + longitudinal continuity) replaces the retired equality gate. **Status 2026-06-21:** arc-primitive valid-by-construction routing + world-anchored run identity + the headless invariance/replay harness (Phases 0–5 of 09-INVARIANCE-HARNESS, in `test/`) all landed; the surface is window-invariant + drivable. v1.1 is now in **bug-polish** — remaining road-surface work is tracked as tickets in `.planning/todos/`, not new phase plans: BUG-12/14/15/16, FEAT-07 (intersections), PERF-02 (stream hitch). Closed this pass: BUG-06/08/10/11, QUAL-03.
...
**Plans**: 12 executed + 5 gap-closure (09-13..16 shipped) + COMBINED LIFECYCLE/CAMBER REFACTOR (09-18..24 — D0 arc-fillet min-radius, D1 generation versioning, D5 hysteresis + D4 arm-disambiguation, D2 shared camber profile, D3 carve cross-section + multi-arm, harness gates, dirt shoulders; supersedes the never-executed 09-14/15) · GAP CLOSURE 09-25..30 (BUG-14/12/10 — continuous per-run profile + single road-query API; closes the 3 remaining in-sim seam bugs)
  ...
  - [x] 09-21-PLAN.md — D2: ONE slew-limited `camberProfile(arcS)` per canonical run (signed-κ → rate limit → ±6°), cached + generation-invalidated; ribbon + physics read it (visual==physics, no clamp-flip spike #4) (SURF-03/04)
  ...
  - [x] 09-26-PLAN.md — P1 API: RoadSample + byArc(runKey,arcS) + sampleRoadAt(x,z) (queryNearest demoted to projector); geometry read from runProfile by arcS; surfaceType/onRoad hooks carried, no feature logic (SURF-04/05)
  ...
  - [~] 09-30-PLAN.md — Verification: 3 headless gates SHIPPED + green (seam-grade, ribbon-edge-weld, camber-across-run). IN-SIM HUMAN-VERIFY FAILED 2026-06-15: BUG-14 teleport-into-air still reproduces crossing tile borders. Root cause: gates bypass the real physics path (queryNearest→runProfile); runProfile silently returns gradeY=0 (road.js:2303) when a queryNearest slice's runKey is absent from this._network (_tiles/_network desync at stream seam) → carve snaps ground → upward launch. Needs a gap-closure round (fix root + extend gate through queryNearest). (SURF-01/03/04/05) — RESOLVED 2026-06-16 (commit 5fe27fd): stale per-run profile caches on positional re-stream; cleared in _streamNetwork; BUG-14 + BUG-10 user-confirmed fixed in-sim.
  - [ ] 09-31-PLAN.md — BUG-12 valid-by-construction centerline (QUAL-03/VBC): make routed centerline XZ min-radius valid AS DRAWN so the ribbon can't fold. VERIFIED ROOT CAUSE (test/diag-minradius-pipeline.mjs): filletMinRadius relaxation leaves a 180° hairpin at ~2m (target 15m) → open-road fold; relaxation+pursuit dead-ends ruled out → use constructive arc/biarc (modify tight spans only, preserve gentle). Safety nets stay (VBC-07); junctions deferred (VBC-08). Tonight (committed): minRadius 12→15 + rd_minr diagnostic + harness + plan. Generator impl = next (needs in-sim verify). (SURF-01/03)
