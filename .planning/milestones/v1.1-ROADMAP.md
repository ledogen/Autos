# Roadmap: RangerSim — v1.0 / v1.1 (ARCHIVED)

> **CLOSED-MILESTONE RECORD — archived 2026-07-29. This is not the live plan.**
> Moved here from `.planning/ROADMAP.md`, where it had been stale since 2026-06-21 and read as
> current. Phases 1–9 are complete; **Phase 10 (POI Hooks) was superseded** by `src/poi.js`,
> FEAT-21 (road POI scatter) and FEAT-46 (story POI pads) rather than executed, and the one open
> `09-31` checkbox (BUG-12 valid-by-construction centerline) was resolved by the arc-primitive
> routing rewrite, not by that plan.
>
> **For live work:** `.planning/todos/pending/` is the source of truth, and
> `.planning/story-mode/MILESTONES.md` is the current roadmap (SM-0 → SM-5).
>
> **Kept because it holds rationale that exists nowhere else** — chiefly the Phase 9 notes on why
> carve-terrain-to-MEET-the-ribbon was abandoned as geometrically unsound and re-architected
> (2026-06-12) into the decal-on-top model the road system still runs on. The plan files in
> `.planning/phases/09-road-surface/` record the steps; this file records why the first attempt
> failed. Sibling: [v1.0-ROADMAP.md](v1.0-ROADMAP.md).
>
> *Note: older `.planning/milestones/v1.0-phases/**` documents cite this file as
> `.planning/ROADMAP.md` (e.g. "ROADMAP §Phase 4 Success Criteria"). Those references now point
> here.*
