...
- [ ] 6DOF rigid body physics using quaternion rotation (no Euler gimbal lock)
- [ ] Fixed physics timestep at 1/60s, deterministic across sessions
- [ ] Three.js Y-up coordinate system throughout — no conversion layers
- [ ] Surface normals accounted for in physics from day one (no flat-ground assumption)
- [ ] 2002 Ford Ranger as default vehicle with real specs (rounded to 3 sig figs)
- [ ] Tire model: Pacejka Magic Formula lateral force vs slip angle
- [ ] Real wheel angular velocity — enables longitudinal slip, wheelspin, burnouts
- [ ] Ackermann steering geometry driven by player input
- [ ] Steering input: accumulated keyboard hold (analog feel from digital input), not bang-bang
- [ ] Spring-damper suspension: tires and body as separate rigid bodies
- [ ] Spring stiffness, damping coefficient, ride height as tunable parameters
- [ ] Open differential RWD drivetrain to start
- [ ] Spring-follow chase camera (multi-mode support in architecture)
- [ ] In-game debug menu (sliders for physics constants, live feedback)
- [ ] Scenario testing system: JSON scenario files → scripted inputs → JSON log output
- [ ] Scenario glossary at docs/GLOSSARY.md defining all terms and sign conventions
- [ ] ES6 module architecture: tire.js, suspension.js, physics.js, vehicle.js, main.js
- [ ] Modular vehicle definition (car specs in a data file, swappable)
- [ ] Rollover-capable physics with a validation ramp prop
...
|----------|-----------|---------|
| Quaternion rotation from day one | Euler angles caused gimbal lock at 90° in prototype — core failure mode to eliminate | — Pending |
| Three.js Y-up everywhere, no SAE conversion | Conversion layers are error-prone across LLM sessions; vector naming handles readability | — Pending |
| ES6 modules (not single HTML file) | Physics complexity requires maintainable separation — tire, suspension, physics, vehicle | — Pending |
| Fixed 1/60s physics timestep | Deterministic scenarios — same input file always produces same log | — Pending |
| Real wheel angular velocity from day one | Required for correct longitudinal slip model; visual-only spin was a known prototype gap | — Pending |
| Surface normals in physics from day one | Infinite terrain is a planned milestone — retrofitting normal support is painful | — Pending |
| Vehicle specs in data file (swappable) | User wants to try different vehicles later — Ranger is default, not hardcoded | — Pending |
| Hand-rolled physics (no library) | User wants tuning transparency and control; physics library would obscure tire/suspension model | — Pending |
